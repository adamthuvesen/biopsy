...
def test_exclude_drops_columns(tmp_path: Path) -> None:
    ...
    excluded_findings = [
        f for f in prof.findings if any(c in f.columns for c in ("status", "constant_col"))
    ]
...
def test_filter_parser_forms() -> None:
    ...
    assert parse_filter_expr("segment in train,test", dtypes) == "\"segment\" IN ('train', 'test')"
...
def test_h3_filter_parser_symbolic_before_keyword() -> None:
    ...
    assert parse_filter_expr("event_name == sign in", dtypes) == "\"event_name\" = 'sign in'"
    # `!=` against a value containing 'in'
    assert parse_filter_expr("desc != foo in bar", dtypes) == "\"desc\" <> 'foo in bar'"
    # Domain ending in '.in' — must be parsed as a string, not split on 'in'
    assert parse_filter_expr("email == johndoe.in", dtypes) == "\"email\" = 'johndoe.in'"
    # Column name containing 'in' as a substring still parses correctly
    dtypes2 = {"training_split": "VARCHAR"}
    assert parse_filter_expr("training_split == train", dtypes2) == "\"training_split\" = 'train'"
    assert (
        parse_filter_expr("training_split in train,test", dtypes2)
...
def test_h1_sample_after_filter(tmp_path: Path) -> None:
    ...
    assert prof.n_rows >= 80, f"sample-after-filter should retain ~100 NO rows; got {prof.n_rows}"
